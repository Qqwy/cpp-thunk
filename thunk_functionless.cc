...
private:
  ...
  constexpr Result visit() noexcept(noexcept(std::declval<Lambda>())) {
    return std::visit([this](auto &&arg) {
      ...
      if constexpr(std::is_same_v<Lambda, T>){
        Result res = arg();
        d_thunk.template emplace<Result>(res);
        return res;
      } else if constexpr(std::is_same_v<Result, T>) {
          return arg;
      } else {
...
template<typename Lam,
         typename Res = std::invoke_result_t<Lam>
         > std::ostream &operator<<(std::ostream &out, Thunk<Lam> const &val)
{
  return out << (val.operator Res());
};
